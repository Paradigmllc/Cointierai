...
import { getYieldsPools } from '@/lib/db/ssot-queries';
...
export default async function YieldsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const rows = await getYieldsPools(200);
          ...
          <TableBody>
            {rows.map((p, i) => (
              <TableRow key={p.pool_id}>
                ...
                <TableCell className="text-right num tabular-nums">${formatCompact(p.tvl_usd)}</TableCell>
