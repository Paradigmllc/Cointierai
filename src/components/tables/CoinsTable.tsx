'use client';

import { useState, useMemo } from 'react';
import Image from 'next/image';
import {
  useReactTable,
  getCoreRowModel,
  getSortedRowModel,
  getPaginationRowModel,
  flexRender,
  type ColumnDef,
  type SortingState,
} from '@tanstack/react-table';
import { useTranslations } from 'next-intl';
import { Link } from '@/i18n/routing';
import { ArrowUp, ArrowDown, Star, ChevronsUpDown } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { TierBadge } from '@/components/coin/TierBadge';
import { formatPrice, formatCompact, formatPercent, formatSupply, changeColor, cn } from '@/lib/utils';
import type { Coin } from '@/types/database';

interface CoinsTableProps {
  data: Coin[];
  pageSize?: number;
  showPagination?: boolean;
}

export function CoinsTable({ data, pageSize = 100, showPagination = true }: CoinsTableProps) {
  const tTable = useTranslations('table');
  const [sorting, setSorting] = useState<SortingState>([{ id: 'rank', desc: false }]);

  const columns = useMemo<ColumnDef<Coin>[]>(
    () => [
      // Watchlist (★)
      {
        id: 'watchlist',
        header: '',
        cell: () => (
          <button
            aria-label="Add to watchlist"
            className="text-muted-foreground hover:text-tier-s transition-colors"
            onClick={(e) => {
              e.preventDefault();
              // TODO: ウォッチリスト追加 (M3 で実装)
            }}
          >
            <Star className="h-3.5 w-3.5" />
          </button>
        ),
        size: 24,
        enableSorting: false,
      },
      // Rank
      {
        id: 'rank',
        accessorKey: 'rank',
        header: tTable('rank'),
        cell: ({ row }) => <span className="num text-muted-foreground text-data-xs">{row.original.rank ?? '—'}</span>,
        size: 40,
      },
      // Tier
      {
        id: 'tier',
        accessorKey: 'tier',
        header: tTable('tier'),
        cell: ({ row }) => <TierBadge tier={row.original.tier} size="sm" />,
        size: 50,
      },
      // Coin (image + name + symbol)
      {
        id: 'name',
        accessorKey: 'name',
        header: tTable('name'),
        cell: ({ row }) => (
          <Link href={`/coin/${row.original.id}`} className="flex items-center gap-2.5 group">
            {row.original.image_url ? (
              <Image
                src={row.original.image_url}
                alt={row.original.symbol}
                width={20}
                height={20}
                className="rounded-full shrink-0"
                unoptimized
              />
            ) : (
              <div className="h-5 w-5 rounded-full bg-muted shrink-0" />
            )}
            <div className="flex flex-col leading-tight">
              <span className="font-medium text-foreground group-hover:text-primary transition-colors">{row.original.name}</span>
              <span className="text-data-xs text-muted-foreground uppercase">{row.original.symbol}</span>
            </div>
          </Link>
        ),
        size: 200,
      },
      // Price
      {
        id: 'price',
        accessorKey: 'price_usd',
        header: tTable('price'),
        cell: ({ row }) => <span className="num font-medium">{formatPrice(row.original.price_usd)}</span>,
        size: 100,
      },
      // 1h
      {
        id: 'change_1h',
        accessorKey: 'change_1h',
        header: tTable('change1h'),
        cell: ({ row }) => (
          <span className={cn('num font-medium', changeColor(row.original.change_1h))}>
            {formatPercent(row.original.change_1h)}
          </span>
        ),
        size: 70,
      },
      // 24h
      {
        id: 'change_24h',
        accessorKey: 'change_24h',
        header: tTable('change24h'),
        cell: ({ row }) => (
          <span className={cn('num font-medium', changeColor(row.original.change_24h))}>
            {formatPercent(row.original.change_24h)}
          </span>
        ),
        size: 80,
      },
      // 7d
      {
        id: 'change_7d',
        accessorKey: 'change_7d',
        header: tTable('change7d'),
        cell: ({ row }) => (
          <span className={cn('num font-medium', changeColor(row.original.change_7d))}>
            {formatPercent(row.original.change_7d)}
          </span>
        ),
        size: 80,
      },
      // Market Cap
      {
        id: 'market_cap',
        accessorKey: 'market_cap_usd',
        header: tTable('marketCap'),
        cell: ({ row }) => <span className="num">{formatCompact(row.original.market_cap_usd)}</span>,
        size: 120,
      },
      // 24h Volume
      {
        id: 'volume_24h',
        accessorKey: 'volume_24h_usd',
        header: tTable('volume24h'),
        cell: ({ row }) => <span className="num">{formatCompact(row.original.volume_24h_usd)}</span>,
        size: 120,
      },
      // Circulating Supply
      {
        id: 'circulating',
        accessorKey: 'circulating_supply',
        header: tTable('circulating'),
        cell: ({ row }) => (
          <span className="num text-muted-foreground text-data-xs">
            {formatSupply(row.original.circulating_supply, row.original.symbol.toUpperCase())}
          </span>
        ),
        size: 140,
      },
    ],
    [tTable],
  );

  const table = useReactTable({
    data,
    columns,
    state: { sorting },
    onSortingChange: setSorting,
    getCoreRowModel: getCoreRowModel(),
    getSortedRowModel: getSortedRowModel(),
    getPaginationRowModel: showPagination ? getPaginationRowModel() : undefined,
    initialState: { pagination: { pageSize } },
  });

  const totalRows = data.length;
  const currentPage = table.getState().pagination.pageIndex + 1;
  const fromRow = (currentPage - 1) * pageSize + 1;
  const toRow = Math.min(currentPage * pageSize, totalRows);

  if (totalRows === 0) {
    return <div className="py-16 text-center text-muted-foreground text-sm">{tTable('noResults')}</div>;
  }

  return (
    <div className="space-y-4">
      <div className="overflow-x-auto thin-scrollbar rounded-lg border border-border/60 bg-card/30">
        <table className="data-table">
          <thead>
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id}>
                {headerGroup.headers.map((header) => {
                  const canSort = header.column.getCanSort();
                  const sortDir = header.column.getIsSorted();
                  return (
                    <th
                      key={header.id}
                      style={{ width: header.getSize() }}
                      className={cn(canSort && 'sortable')}
                      onClick={canSort ? header.column.getToggleSortingHandler() : undefined}
                    >
                      <span className="inline-flex items-center gap-1">
                        {flexRender(header.column.columnDef.header, header.getContext())}
                        {canSort && (
                          <span className="inline-flex shrink-0">
                            {sortDir === 'asc' ? (
                              <ArrowUp className="h-3 w-3" />
                            ) : sortDir === 'desc' ? (
                              <ArrowDown className="h-3 w-3" />
                            ) : (
                              <ChevronsUpDown className="h-3 w-3 opacity-30" />
                            )}
                          </span>
                        )}
                      </span>
                    </th>
                  );
                })}
              </tr>
            ))}
          </thead>
          <tbody>
            {table.getRowModel().rows.map((row) => (
              <tr key={row.id}>
                {row.getVisibleCells().map((cell) => (
                  <td key={cell.id}>{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>
                ))}
              </tr>
            ))}
          </tbody>
        </table>
      </div>

      {showPagination && (
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span>{tTable('showing', { from: fromRow, to: toRow, total: totalRows })}</span>
          <div className="flex items-center gap-2">
            <Button size="xs" variant="outline" onClick={() => table.previousPage()} disabled={!table.getCanPreviousPage()}>
              {tTable('previous')}
            </Button>
            <Button size="xs" variant="outline" onClick={() => table.nextPage()} disabled={!table.getCanNextPage()}>
              {tTable('next')}
            </Button>
          </div>
        </div>
      )}
    </div>
  );
}
