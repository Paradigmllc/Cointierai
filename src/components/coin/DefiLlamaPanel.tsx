...
interface DefiLlamaPanelProps {
  slug: string;
  /** Current aggregate already materialized into coins (used for at-a-glance KPIs). */
...
/**
 * DeFiLlama deep-dive panel. Renders:
 *  - KPI strip: TVL now, 1d/7d/30d changes, category, audit/twitter chips
 *  - Area chart: 90-day TVL trend
 *  - Stacked bar: per-chain TVL breakdown (top 7 chains)
 *  - Hallmarks list (significant events)
 *
 * Server component — `getProtocolDetail` is called inline and the rendered
 * HTML ships in the initial page response. Any fetch error degrades to an
 * empty section rather than blocking the rest of the page.
 */
export async function DefiLlamaPanel(props: DefiLlamaPanelProps) {
  return (
    <Suspense fallback={null}>
      <DefiLlamaPanelInner {...props} />
    </Suspense>
  );
}

async function DefiLlamaPanelInner({ slug, currentTvlUsd, change1d, change7d, category, locale }: DefiLlamaPanelProps) {
  let detail;
  try {
    detail = await getProtocolDetail(slug);
  } catch {
    return null;
  }
  ...
  // Per-chain breakdown — take the latest TVL for each chain key.
  const chainTvls = Object.entries(detail.chainTvls ?? {})
    .filter(([k]) => !k.includes('-')) // drop derived "Ethereum-borrowed" etc.
    .map(([chain, v]) => {
